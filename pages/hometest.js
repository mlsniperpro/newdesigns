import Card from '../components/newDesign/Card';
import Footer from '@/components/newDesign/Footer';
import Hero from '@/components/newDesign/Hero';
import PromptsDir from '@/components/newDesign/PromptsDir';
import Video from '@/components/newDesign/Video';
...
const paragraphs = [
  ...
  <div key="p7">
    ...
    <ul>
      <li key="li1">
        Tendrás acceso a un registro histórico de conversaciones que te
        permitirá entender mejor las necesidades de tus contactos.{' '}
      </li>
      <li key="li2">
        Tiene la función inteligente de botón que permite redireccionar las
        conversaciones del chatbot a tu sistema de citas de Vioniko, a tu
        carrito de compras, a tu Telegram o WhatsApp o donde tu dispongas
      </li>
    </ul>
  </div>,
];
    ...
    <main className="">
      ...
      {headings.map((heading, index) => (
        <Card
          ...
          subheading={
            index === paragraphs.length - 2 || index === paragraphs.length - 1
              ? '(SOLO DISPONIBLE EN LA VERSION SDE PAGA)'
              : ''
          }
        >
          {paragraphs[index]}
